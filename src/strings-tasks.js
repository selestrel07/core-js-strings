...
function removeLastOccurrences(str, value) {
  ...
  return ind === -1
    ? str
    : str.slice(0, ind) + str.slice(ind).replace(value, '');
}
...
function containsSubstring(str, substring) {
  return str != null ? str.includes(substring) : false;
}
...
function encodeToRot13(str) {
  const ord = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz !?,.';
  const syph = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm !?,.';
  return str
    .split('')
    .map((ch) => syph.charAt(ord.indexOf(ch)))
    .join('');
}
